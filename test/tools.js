...
function getSubId() {
  return Math.random().toFixed(64).replace(".", "x");
}

async function processSubscription(raiseContract, user, issuer, shares, approve) {
  const sub = { val: shares, approved: approve, id: getSubId() };
  await raiseContract.subscribe(sub.id, sub.val, { from: user });
  if (sub.approved !== "pending")
    await raiseContract.issuerSubscription(sub.id, sub.approved, {
      from: issuer,
    });
  return sub;
}
